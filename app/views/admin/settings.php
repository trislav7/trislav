        ...
        <form method="POST" class="space-y-8">
            ...
            <div class="bg-secondary rounded-xl p-6 border border-highlight/30">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block mb-3 font-medium text-light">Email</label>
                        <input type="email" name="setting_email"
                               value="<?= htmlspecialchars($settings['email'] ?? '') ?>"
                               class="w-full px-4 py-3 bg-primary border border-highlight/30 rounded-lg text-light transition-colors duration-300 focus:outline-none focus:border-highlight"
                               placeholder="info@example.com">
                    </div>
                    <div>
                        <label class="block mb-3 font-medium text-light">Telegram</label>
                        <input type="text" name="setting_telegram"
                               value="<?= htmlspecialchars($settings['telegram'] ?? '@alexmarinkov') ?>"
                               class="w-full px-4 py-3 bg-primary border border-highlight/30 rounded-lg text-light transition-colors duration-300 focus:outline-none focus:border-highlight"
                               placeholder="@username">
                        <p class="text-gray-400 text-sm mt-2">
                            Введите username Telegram (например: @alexmarinkov)
                        </p>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block mb-3 font-medium text-light">VK URL *</label>
                        <input type="url" name="setting_vk_url"
